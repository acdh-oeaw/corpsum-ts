	...
	<form class="mt-4 flex max-w-2xl flex-col gap-4" @submit.prevent="form.handleSubmit">
		...
		>
			<div class="grid gap-2">
				...
				<p v-if="hasError(state)" id="noske-error" class="text-xs text-destructive" role="alert">
					{{ firstError(state) }}
				</p>
			</div>
		</component>
		...
		>
			<div class="grid gap-2">
				...
				<p v-if="hasError(state)" id="corpus-error" class="text-xs text-destructive" role="alert">
					{{ firstError(state) }}
				</p>
			</div>
		</component>
