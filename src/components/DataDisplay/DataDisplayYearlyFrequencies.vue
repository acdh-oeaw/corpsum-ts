...
const q = computed(() => queries.value.map((query, index) => {
	return {
		queryKey: ["get-yearly-frequencies", query.corpus, query.subCorpus, query.finalQuery] as const,
		queryFn: async () => {
		...
		select: (data: Type11Freqml) => {
			yearlyFrequencies.value[index] = data.Blocks?.map((block) => block.Items?.map(item => {
					return {
						absolute: item.frq!,
						relative: item.reltt!,
